...
class Agent():
    ...
    def __init__(self, hyperparameter_set):
        ...
        # Create run-specific directory with environment name and timestamp
        self.run_timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.run_dir = os.path.join(RUNS_DIR, f"{self.env_id}_{self.run_timestamp}")
        os.makedirs(self.run_dir, exist_ok=True)
        
        # Create video directory within the run directory
        self.video_dir = os.path.join(self.run_dir, "videos")
        os.makedirs(self.video_dir, exist_ok=True)

        # Path to Run info
        self.LOG_FILE   = os.path.join(self.run_dir, f'{self.hyperparameter_set}.log')
        self.MODEL_FILE = os.path.join(self.run_dir, f'{self.hyperparameter_set}.pt')
    ...
    def run(self, is_training=True, render=False):
        ...
        env = gym.make(self.env_id, render_mode='rgb_array', **self.env_make_params)
        # Wrap with video recorder
        env = RecordVideo(env, video_folder=self.video_dir, name_prefix=f"{self.env_id}_{self.hyperparameter_set}")
        ...
        if is_training:
            ...
            # Enable automatic mixed precision for GPU training
            if device == 'cuda':
        ...
        for episode in itertools.count():
            ...
            if device == 'cuda' and episode % 100 == 0:
                torch.cuda.empty_cache()
                ...
                if current_time - last_graph_update_time > timedelta(seconds=10):
                    ...
                    if device == 'cuda':
                        ...
                        if memory_allocated > 0.8 * torch.cuda.get_device_properties(0).total_memory / 1024**3:
                            torch.cuda.empty_cache()
                            print("GPU cache cleared due to high memory usage")
    ...
    def optimize(self, mini_batch, policy_dqn, target_dqn):
        ...
        # Optimize the model (backpropagation) with mixed precision for GPU
        self.optimizer.zero_grad()  # Clear gradients
        
        if device == 'cuda' and hasattr(self, 'scaler'):
            # Use automatic mixed precision for GPU training
            self.gpu_start_event.record()
            with torch.amp.autocast('cuda'):
                loss = self.loss_fn(current_q, target_q)
            self.scaler.scale(loss).backward()  # Scale loss and compute gradients
            self.scaler.step(self.optimizer)    # Update network parameters
